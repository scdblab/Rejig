...
public class MemcachedClient {
	...
	private Map<String, Long> i_lease_list;
	private Map<String, Long> q_lease_list;

	public static int maxBackoff = 0;
	private AtomicInteger localConfigurationNumber = new AtomicInteger(-1);
	 ...
	 * Creates a new instance of MemCachedClient.
	 */
	public MemcachedClient(long backOff) {
		this.backOff = backOff;
		init();
	}
	
	public void updateLocalConfigurationNumber(int configNumber) {
		this.localConfigurationNumber.set(configNumber);
	}
	 ...
	 *            name of SockIOPool
	 */
	public MemcachedClient(String poolName, long backOff) {
		this.poolName = poolName;
		this.backOff = backOff;
		init();
	}
	...
	private void init() {
		...
		this.i_lease_list = new ConcurrentHashMap<String, Long>();
		this.q_lease_list = new ConcurrentHashMap<String, Long>();
	...
	/**
	 * Sets default String encoding when storing primitives as Strings. Default is
	 * UTF-8.
	 * 
	...
	public boolean keyExists(String key) {
		return (this.get(key, null, true, -1) != null);
	}
	...
	public boolean quarantineAndRegister(String tid, String key, Integer hashCode) throws Exception {
		...
		// build command
		StringBuilder command = new StringBuilder("qareg ").append(localConfigurationNumber.get() + " ")
				.append(key + " ").append(tid);
		command.append("\r\n");
	...
	public Object ciget(String sid, String key, boolean asString) throws IOException, COException {
			...
			try {
				...
				while (true) {
					...
					if (line.startsWith(VALUE)) {
						...
						if (backOff == 0) {
							throw new COException("The server asks for retry");
						} else
							break;
					} else if (line.startsWith(ABORT)) {
						value_found = false;

						throw new COException("ciget Session aborted.");
		...
		try {
			...
			while (true) {
				...
				if (line.startsWith(VALUE)) {
					...
					socket.close();
					socket = null;
					throw new COException("Failed OQRead to quarantine key: " + key);
				} else if (line.startsWith(ABORT)) {
					socket.close();
					socket = null;
					throw new COException("Session was aborted.");
				} else if (END.equals(line)) {
					if (log.isDebugEnabled())
						log.debug("++++ finished reading from cache server");
					break;
	...
	private boolean oqWrite(String sid, String key, Object value, Date expiry, Integer hashCode, boolean asString)
		...
		try {
			...
			if (OK.equals(line)) {
				...
				throw new COException("oqswap session was aborted.");
			} else if (INVALID.equals(line)) {
				if (log.isInfoEnabled())
					log.info("++++ sar ignored for key: " + key);
				throw new COException("oqwrite session was aborted.");
			} else {
	...
	public boolean validate(String sid) throws COException {
		...
		try {
			...
			if (OK.equals(line)) {
				...
				sock.close();
				sock = null;
				throw new COException("validate Session aborted.");
			} else {
	...
	public boolean unleaseCO(String sid) throws COException {
		...
		try {
			...
			if (OK.equals(line)) {
				...
				throw new COException("unleaseCO Session aborted.");
			} else {
	...
	public boolean oqReg(String sid, String key, Integer hashCode) throws COException {
		...
		try {
			...
			if (OK.equals(line)) {
				...
				sock.close();
				sock = null;
				throw new COException("oqreg aborted.");
			} else {
	...
	public boolean dCommit(String sid, Integer hashCode) throws COException {
		...
		try {
			...
			if (OK.equals(line)) {
				...
				throw new COException("dCommit session aborted.");
			} else {
	...
	private boolean sendDeleteAndRelease(String tid, Integer hashCode) throws Exception {
		...
		// build command
		StringBuilder command = new StringBuilder("dar ").append(localConfigurationNumber.get() + " ").append(tid);
		command.append("\r\n");
	...
	public boolean commit(String tid) throws Exception {
		if (tid == null) {
			log.error("null value for key passed to deleteAndRelease()");
			return false;
		}
		...
		// get SockIO obj from hash or from key
		SockIOPool.SockIO sock = getSocket(tid, null);

		// build command
		StringBuilder command = new StringBuilder("commit ").append(tid);
		command.append("\r\n");
	...
	public boolean ewcommit(String tid) {
		return ewcommit(tid, null, -1, true);
	}

	public boolean ewcommit(String tid, int fragCfgId, boolean pending) {
		return ewcommit(tid, null, fragCfgId, pending);
	}

	public boolean ewcommit(String tid, Integer hashCode, boolean pending) {
		return ewcommit(tid, hashCode, -1, pending);
	}
	 ...
	 * @return
	 */
	public boolean ewcommit(String tid, Integer hashCode, int fragCfgId, boolean pending) {
		...
		// build command
		int p = pending ? 1 : 0;
		StringBuilder command = new StringBuilder("commit ").append(String.valueOf(localConfigurationNumber.get()))
				.append(" ").append(fragCfgId).append(" ").append(tid).append(" ").append(p);
		command.append("\r\n");
	...
	public boolean delete(String key) throws Exception {
		return delete(key, null, null, true);
	}

	public boolean delete(String key, boolean delete_lease) throws Exception {
		return delete(key, null, null, delete_lease);
	}
	...
	public boolean delete(String key, Date expiry) throws Exception {
		return delete(key, null, expiry, true);
	}

	/**
	 * Deletes an object from cache given cache key, a delete time, and an optional
	 * hashcode.
	 *
	 ...
	 * @throws Exception
	 */
	public boolean delete(String key, Integer hashCode, Date expiry, boolean delete_lease) throws Exception {
		...
		// build command
		StringBuilder command = new StringBuilder("delete ").append(localConfigurationNumber.get());
		if (delete_lease) {
			command.append(" 1");
		} else {
			command.append(" 0");
	...
	public void deleteMulti(List<String> keys, boolean delete_lease) throws Exception {
		...
		// get SockIO obj from hash or from key
		SockIOPool.SockIO sock = getSocket(keys.get(0), null);
		...
		try {
			...
			if (DELETED.equals(line)) {
				if (log.isInfoEnabled())
					log.info("++++ deletion of keys: " + keys + " from cache was a success");
				...
				if (log.isInfoEnabled())
					log.info("++++ deletion of keys: " + keys + " from cache failed as the key was not found");
			} else if (FAIL.equals(line)) {
				sock.close();
				sock = null;
				throw new StaleConfigurationException();
			} else {
				log.error("++++ error deleting keys: " + keys);
				log.error("++++ server response: " + line);

				sock.close();
				sock = null;
				throw new Exception(
						"Server error on Delete request (" + keys + "): " + line + " \nCommand = " + command);
			}
		} catch (IOException e) {
			// exception thrown
			log.error("++++ exception thrown while writing bytes to server on delete");
	...
	public boolean updateServerConfigurationNumber(int newConfigNumber) throws Exception {

		// get SockIO obj from hash or from key
		SockIOPool.SockIO sock = getSocket(String.valueOf(newConfigNumber), 0);

		// build command
		StringBuilder command = new StringBuilder("updateconf ").append(newConfigNumber);
		command.append("\r\n");
		...
		try {
			...
			if (OK.equals(line)) {
				...
				if (log.isInfoEnabled())
					log.info("++++ updateconf: " + newConfigNumber + " from cache was a success");
				...
				if (log.isInfoEnabled())
					log.info(
							"++++ deletion of key: " + newConfigNumber + " from cache failed as the key was not found");
			} else {
				log.error("++++ error deleting key: " + newConfigNumber);
				log.error("++++ server response: " + line);

				sock.close();
				sock = null;
				throw new Exception("Server error on Delete request (" + newConfigNumber + "): " + line
						+ " \nCommand = " + command);
			}
		} catch (IOException e) {
			...
			// if we have an errorHandler, use its hook
			if (errorHandler != null)
				errorHandler.handleErrorOnDelete(this, e, String.valueOf(newConfigNumber));
	...
	/***
	 * Read the key and attempt to Quarantine it. The Quarantine may fail if another
	 * thread/client holds a Q-lease on the key. In this case, an IQException is
	 * thrown. If the value for this key exists in the cache, it is returned. If
	 * not, null is returned. A null value indicates that the Quarantine was
	 * successful.
	 * 
	 * @param key
	...
	public Object quarantineAndRead(String key) throws IQException, Exception {
		return quarantineAndRead(key, true);
	}

	public Object quarantineAndRead(String key, boolean readValue) throws IQException, Exception {
		return quarantineAndRead(key, null, false, readValue);
	}
	...
	public Object quarantineAndRead(String key, Integer hashCode, boolean asString, boolean readValue)
			throws IQException {
		CLValue result = null;
		...
		try {
			...
			// actual
			// logic.
			result = quarantineAndReadMain(null, key, hashCode, asString, readValue);
		} catch (IQException e) {
			releaseAllLeases();
			throw e;
		}
	...
	public CLValue ewread(String tid, String key, Integer hashCode, boolean asString) throws IQException {
		return quarantineAndReadMain(tid, key, hashCode, asString, true);
	}

	public CLValue ewreadvalue(String tid, String key) throws IQException {
		return quarantineAndReadMain(tid, key, null, false, true);
	}

	public void ewread(String tid, String key) throws IQException {
		quarantineAndReadMain(tid, key, null, false, false);
	}

	public boolean ewswap(String tid, String key, Object value) throws IQException {
		boolean ret = false;
		try {
			ret = swap(key, null, value, -1);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public boolean ewswap(String tid, String key, Object value, int fragCfgId) throws IQException {
		boolean ret = false;
		try {
			ret = swap(key, null, value, fragCfgId);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public boolean swap(String key, Integer hashCode, Object value) throws IOException, IQException {
		return swap(key, hashCode, value, -1);
	}

	public boolean swap(String key, Integer hashCode, Object value, int fragCfgId) throws IOException, IQException {
		if (key == null) {
			log.error("key is null or cmd is null/empty for set()");
			return false;
		}
		...
		try {
			...
			cmd = String.format("%s %d %d %s %d %d %d %d\r\n", "swap", localConfigurationNumber.get(), fragCfgId, key,
					flags, (expiry.getTime() / 1000), val.length, current_token);
	...
	public Object quarantineAndRead(String tid, String key, Integer hashCode, boolean asString) throws IQException {
		...
		try {
			...
			// actual
			// logic.
			result = quarantineAndReadMain(tid, key, hashCode, asString, true);
		} catch (IQException e) {
			releaseAllLeases();
			throw e;
		}
	...
	private CLValue quarantineAndReadMain(String tid, String key, Integer hashCode, boolean asString, boolean readValue)
			throws IQException {
		int pending = 0;
		...
		// get SockIO obj using cache key
		SockIOPool.SockIO socket = null;
		String cmd = String.format("qaread %d %s %d", localConfigurationNumber.get(), key, readValue ? 1 : 0);
	...
	private SockIOPool.SockIO getSocket(String key, Integer hashCode) {
		...
		while (true) {
			...
			if (sock == null) {
				...
				System.out.println("no socket available " + poolName);
				System.exit(0);
				try {
					Thread.sleep(100);
				} catch (InterruptedException e) {
	...
	private boolean releaseX(String key, Long lease_token, Integer hashCode) {
		...
		StringBuilder command = new StringBuilder("unlease ").append(localConfigurationNumber.get()).append(" ")
				.append(key);
	...
	public CASResponse cas(String key, Object value, long casToken) throws IOException {
		return casResponse(
				set("cas", key, value, null, null, primitiveAsString, Optional.of(casToken), Optional.empty(), -1));
	}
	...
	public CASResponse ccas(String key, Object value, long casToken, long cost) throws IOException {
		return casResponse(
				set("ccas", key, value, null, null, primitiveAsString, Optional.of(casToken), Optional.of(cost), -1));
	}
	...
	public CASResponse cas(String key, Object value, long casToken, Integer hashCode) throws IOException {
		return casResponse(
				set("cas", key, value, null, hashCode, primitiveAsString, Optional.of(casToken), Optional.empty(), -1));
	}
	...
	public CASResponse cas(String key, Object value, Date expiry, long casToken) {
		try {
			return casResponse(set("cas", key, value, expiry, null, primitiveAsString, Optional.of(casToken),
					Optional.empty(), -1));
		} catch (Exception e) {
		}
	...
	public CASResponse ccas(String key, Object value, Date expiry, long casToken, long cost) {
		try {
			return casResponse(set("ccas", key, value, expiry, null, primitiveAsString, Optional.of(casToken),
					Optional.of(cost), -1));
		} catch (Exception e) {
		}
	...
	public CASResponse cas(String key, Object value, Date expiry, long casToken, Integer hashCode) throws IOException {
		return casResponse(set("cas", key, value, expiry, hashCode, primitiveAsString, Optional.of(casToken),
				Optional.empty(), -1));
	}
	...
	public boolean set(String key, Object value) throws IOException {
		return setResponse(
				set("set", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean cset(String key, Integer hashCode, Object value, long cost) throws IOException {
		return setResponse(
				set("cset", key, value, null, hashCode, primitiveAsString, Optional.empty(), Optional.of(cost), -1));
	}
	...
	public boolean set(String key, Object value, Integer hashCode) throws IOException {
		return setResponse(
				set("set", key, value, null, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean set(String key, Object value, Date expiry) throws IOException {
		try {
			return setResponse(
					set("set", key, value, expiry, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
		} catch (Exception e) {
		}
	...
	public boolean set(String key, Object value, Date expiry, Integer hashCode) throws IOException {
		return setResponse(
				set("set", key, value, expiry, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}

	public boolean append(String key, Integer hashCode, Object value) throws IOException {
		return setResponse(
				set("append", key, value, null, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}

	public boolean append(String key, Object value) throws IOException {
		return setResponse(
				set("append", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}

	public boolean cappend(String key, Object value, long cost) throws IOException {
		return setResponse(
				set("cappend", key, value, null, null, primitiveAsString, Optional.empty(), Optional.of(cost), -1));
	}

	public boolean prepend(String key, Object value) throws IOException {
		return setResponse(
				set("prepend", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean iqset(String key, Object value) throws IOException, IQException {
		return setResponse(
				set("iqset", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	 ...
	 * @throws IQException
	 */
	public boolean iqset(String key, Object value, int nextFragmentCfgId) throws IOException, IQException {
		return setResponse(set("iqset", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(),
				nextFragmentCfgId));
	}
	...
	public boolean iqset(String key, Object value, Date expiry) throws IOException, IQException {
		return setResponse(
				set("iqset", key, value, expiry, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean iqset(String key, Object value, Date expiry, Integer hashCode) throws IOException, IQException {
		return setResponse(
				set("iqset", key, value, expiry, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean add(String key, Object value) throws IOException {
		return setResponse(
				set("add", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean cadd(String key, Object value, long cost) throws IOException {
		return setResponse(
				set("cadd", key, value, null, null, primitiveAsString, Optional.empty(), Optional.of(cost), -1));
	}

	/**
	 * Adds data to the server; the key, value, and an optional hashcode are passed
	 * in.
	 *
	 * @param key
	 *            key to store data under
	...
	public boolean add(String key, Object value, Integer hashCode) throws IOException {
		return setResponse(
				set("add", key, value, null, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean add(String key, Object value, Date expiry) throws IOException {
		return setResponse(
				set("add", key, value, expiry, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean add(String key, Object value, Date expiry, Integer hashCode) throws IOException {
		return setResponse(
				set("add", key, value, expiry, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean replace(String key, Object value) throws IOException {
		return setResponse(
				set("replace", key, value, null, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}

	/**
	 * Updates data on the server; only the key and the value and an optional hash
	 * are specified.
	 *
	 * @param key
	 *            key to store data under
	...
	public boolean replace(String key, Object value, Integer hashCode) throws IOException {
		return setResponse(
				set("replace", key, value, null, hashCode, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean replace(String key, Object value, Date expiry) throws IOException {
		return setResponse(
				set("replace", key, value, expiry, null, primitiveAsString, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean replace(String key, Object value, Date expiry, Integer hashCode) throws IOException {
		return setResponse(set("replace", key, value, expiry, hashCode, primitiveAsString, Optional.empty(),
				Optional.empty(), -1));
	}
	 ...
	 *
	 * If data does not already exist for this key on the server, or if the key is
	 * being<br/>
	 * deleted, the specified value will not be stored.<br/>
	 * The server will automatically delete the value when the expiration time has
	 * been reached.<br/>
	 * <br/>
	...
	private Optional<String> set(String cmdname, String key, Object value, Date expiry, Integer hashCode,
			boolean asString, Optional<Long> casToken, Optional<Long> cost, int nextFragmentCfgId) {
		...
		try {
			...
			cmdBuilder.append(String.format("%s %d %d %s %d %d %d", cmdname, localConfigurationNumber.get(),
					nextFragmentCfgId, key, flags, (expiry.getTime() / 1000), val.length));
			...
			if (cmdname.contains("iqset")) {
				Long current_token = this.i_lease_list.get(key);
				if (current_token == null) {
					throw new IOException("iqset request with no lease token");
				}

				if (current_token.longValue() == -1) {
					return Optional.empty(); // iqget is iqget same session
				}

				cmdBuilder.append(" ");
				cmdBuilder.append(current_token.longValue());
			}
	...
	public boolean storeCounter(String key, long counter) throws IOException {
		return setResponse(
				set("set", key, new Long(counter), null, null, true, Optional.empty(), Optional.empty(), -1));
	}
	 ...
	 */
	public boolean storeCounter(String key, Long counter) throws IOException {
		return setResponse(set("set", key, counter, null, null, true, Optional.empty(), Optional.empty(), -1));
	}
	...
	public boolean storeCounter(String key, Long counter, Integer hashCode) throws IOException {
		return setResponse(set("set", key, counter, null, hashCode, true, Optional.empty(), Optional.empty(), -1));
	}
	...
	public long getCounter(String key, Integer hashCode) {
		...
		long counter = -1;
		try {
			counter = Long.parseLong((String) get(key, hashCode, true, -1));
		} catch (Exception ex) {
	...
	public long addOrIncr(String key, long inc, Integer hashCode) throws IOException {
		boolean ret = false;
		ret = setResponse(set("add", key, new Long(inc), null, hashCode, true, Optional.empty(), Optional.empty(), -1));
	...
	/**
	 * Increment the value at the specified key by the specified increment, and then
	 * return it.
	 *
	 * @param key
	 *            key where the data is stored
	...
	/**
	 * Decrement the value at the specified key by passed in value, and then return
	 * it.
	 *
	 * @param key
	 *            key where the data is stored
	...
	/**
	 * Decrement the value at the specified key by the specified increment, and then
	 * return it.
	 *
	 * @param key
	 *            key where the data is stored
	...
	public Object get(String key, int fragmentCfgId) {
		return get(key, null, false, fragmentCfgId);
	}

	public Object get(String key) {
		return get(key, null, false, -1);
	}
	...
	public Object iset(String key, int maxBackoff) throws IOException {
		return iqget(key, "iset", null, false, maxBackoff, -1, -1, true);
	}

	public CLValue iqget(String key, int maxBackoff, boolean foreground) throws IOException {
		return iqget(key, "iqget", null, false, maxBackoff, -1, -1, foreground);
	}

	public Object iset(String key) throws IOException {
		return iqget(key, "iset", null, false, Integer.MAX_VALUE, -1, -1, true);
	}

	public CLValue iqget(String key) throws IOException {
		return iqget(key, "iqget", null, false, Integer.MAX_VALUE, -1, -1, true);
	}

	public CLValue iqget(String key, boolean foreground) throws IOException {
		return iqget(key, "iqget", null, false, Integer.MAX_VALUE, -1, -1, foreground);
	}

	public CLValue iqget(String key, int maxBackoff, int partitionConfig, int recoverConfig, boolean foreground)
			throws IOException {
		return iqget(key, "iqget", null, false, maxBackoff, partitionConfig, recoverConfig, foreground);
	}
	 ...
	 * @throws IOException
	 */
	public CLValue iqget(String key, Integer hashCode) throws IOException {
		return iqget(key, "iqget", hashCode, false, Integer.MAX_VALUE, -1, -1, false);
	}
	 ...
	 * @throws IOException
	 */
	public CLValue iqget(String key, String command, Integer hashCode, boolean asString, int maxBackoff,
			int partitionConfigNumber, int recoverConfig, boolean foreground) throws IOException {
		if (key == null) {
			log.error("key is null for get()");
			return null;
		}
		...
		// get SockIO obj using cache key
		SockIOPool.SockIO socket = null;
		String cmd = command + " " + localConfigurationNumber.get() + " " + key;
		...
		while (!value_found) {
			...
			try {
				...
				while (true) {
					...
					if (line.startsWith(VALUE)) {
						String[] info = line.split(" ");
						int flag = Integer.parseInt(info[2]);
						pending = Integer.parseInt(info[3]) == 1 ? true : false;
						int length = Integer.parseInt(info[4]);
						configNumber = Integer.parseInt(info[5]);

						if (log.isDebugEnabled()) {
							log.debug("++++ key: " + key);
							log.debug("++++ flags: " + flag);
							log.debug("++++ length: " + length);
							log.debug("++++ pending: " + info[3]);
							log.debug("++++ cfgId: " + configNumber);
						}
			...
			if (cnt > maxBackoff) {
				throw new MaxBackOffExceededException();
			}
		}

		if (lease_found) {
			return new CLValue(null, pending);
		}

		if (o != null) {
			if (partitionConfigNumber == -1) {
				return new CLValue(o, pending);
			}
			if (partitionConfigNumber <= configNumber) {
				return new CLValue(o, pending);
			}
			if (recoverConfig != -1 && recoverConfig <= configNumber) {
				return new CLValue(o, pending);
			}
	...
	public Boolean oqappend(String sid, String key, Object value) throws COException, IOException {
		return oqappendorprepend("oqappend", null, key, value, false, sid);
	}

	public Boolean oqappend(String sid, String key, Integer hashCode, Object value) throws COException, IOException {
		return oqappendorprepend("oqappend", hashCode, key, value, false, sid);
	}
	...
	public CLValue ewappend(String key, Integer hashCode, Object value, String tid) throws IQException {
		try {
		...
		try {
			...
			while (true) {
				...
				if (line.contains(NOVALUE)) {
					...
					sock.close();
					throw new COException("oqincrdecr aborted.");
				} else if (END.equals(line)) {
					break;
				} else {
	...
	private Boolean oqappendorprepend(String cmdname, Integer hashCode, String key, Object value, boolean asString,
		...
		try {
			...
			if (OK.equals(line)) {
				...
				sock.close();
				sock = null;
				throw new COException("oqappend_prepend session aborted.");
			} else {
	...
	private CLValue iqappendorprepend(String cmdname, Integer hashCode, String key, Object value, boolean asString,
		...
		try {
			...
			cmd = String.format("%s %d %s %d %s\r\n", cmdname, localConfigurationNumber.get(), key, val.length, tid);
	...
	public Map<String, Object> getMulti(Integer hashCode, boolean asString, String... keys) {
		...
		cmd.append("get ");
		cmd.append(localConfigurationNumber.get());
		Map<String, String> sanitizedKeyOKeyMap = Maps.newHashMap();

		for (String key : keys) {
			try {
				String sKey = sanitizeKey(key);
				sanitizedKeyOKeyMap.put(sKey, key);
				cmd.append(" " + sKey);
			} catch (UnsupportedEncodingException e) {
		...
		try {
			if (log.isDebugEnabled())
				log.debug("++++ memcache command: " + cmdStr);
			...
			while (true) {
				...
				if (line.startsWith(VALUE)) {
					...
					if (o != null) {
						result.put(sanitizedKeyOKeyMap.get(key), o);
					}
				} else if (END.equals(line)) {
					if (log.isDebugEnabled())
						log.debug("++++ finished reading from cache server");
			...
			System.out.println("IO exception get " + keys + ": ");
			e.printStackTrace();
	 ...
	 *         previously stored
	 */
	public Object get(String key, Integer hashCode, boolean asString, int fragmentCfgId) {
		...
		cmd = "get ";
		cmd += localConfigurationNumber.get() + " ";
		cmd += key + "\r\n";
		...
		try {
			...
			while (true) {
				...
				if (line.startsWith(VALUE)) {
					...
					int flag = Integer.parseInt(info[2]);
					int length = Integer.parseInt(info[3]);
					configNumber = Integer.parseInt(info[4]);
		...
		if (o != null && fragmentCfgId != -1 && fragmentCfgId > configNumber) {
			throw new InvalidValueException();
		}
	 ...
	 * @param keys
	 *            String array of keys to retrieve
	 * @return a hashmap with entries for each key is found by the server, keys that
	 *         are not found are not entered into the hashmap, but attempting to
			...
			if (line.startsWith(VALUE)) {
				...
				if ((flag & F_SERIALIZED) != F_SERIALIZED) {
					...
					try {
						...
						/*
						 * Errors de-serializing are to be expected in the case of a long running server
						 * that spans client restarts with updated classes.
						 */
						// if we have an errorHandler, use its hook
						if (errorHandler != null)
							errorHandler.handleErrorOnGet(this, e, key);
	 ...
	 * Invalidates the entire cache.
	 *
	 * Will return true only if succeeds in clearing all servers. If pass in null,
	 * then will try to flush all servers.
	 * 
	 * @param servers
	 ...
	 * 
	 * @param servers
	 *            string array of servers to retrieve stats from, or all if this is
	 *            null
	 * @return Stats map
	 */
	@SuppressWarnings("rawtypes")
