...
public class SockIOPool {
	...
	public static Map<String,SockIOPool> pools =
	...
	private int debugTOMultiplier     = 100;
	private int poolMultiplier        = 3;
	private int initConn              = 10;
	private int minConn               = 5;
	private int maxConn               = 100;
	private long maxIdle              = 1000 * 60 * 5 * debugTOMultiplier;		// max idle time for avail sockets
	private long maxBusyTime          = 1000 * 30 * debugTOMultiplier;			// max idle time for avail sockets
	private long maintSleep           = 1000 * 30 * debugTOMultiplier;			// maintenance thread sleep time
	private int socketTO              = 1000 * 3 * debugTOMultiplier;			// default timeout of socket reads
	private int socketConnectTO       = 1000 * 3 * debugTOMultiplier;	        // default timeout of socket connections
	...
	public void initialize() {
		...
		synchronized( this ) {
			...
			// pools
			availPool   = new ConcurrentHashMap<String,Map<SockIO,Long>>( servers.length * initConn );
			busyPool    = new ConcurrentHashMap<String,Map<SockIO,Long>>( servers.length * initConn );
	...
	public SockIO getSock( String key, Integer hashCode ) {
		...
		if ( log.isDebugEnabled() ) {
			if (hashCode != null) {
				...
				log.debug( "cache socket pick " + key + " null"  );
			}
		}
	...
	protected void addSocketToPool( Map<String,Map<SockIO,Long>> pool, String host, SockIO socket ) {
		...
		Map<SockIO,Long> sockets =
				...
				new ConcurrentHashMap<SockIO,Long>();
	...
	protected void selfMaint() {
		...
		synchronized( deadPool ) {
			...
			deadPool = new ConcurrentHashMap<SockIO, Integer>();//new IdentityHashMap<SockIO,Integer>();
		}
...
//				throw new IOException( errMsg.toString() );
	...
	public String getServer(Integer hashCode) {
		return this.servers[0];
//		long bucket = getBucket(null, hashCode);
//		return ( this.hashingAlg == CONSISTENT_HASH )
//				? consistentBuckets.get( bucket )
//						: buckets.get( (int)bucket );
	}
}
